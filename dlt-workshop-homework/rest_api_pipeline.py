...
@dlt.source(name="logfire", max_table_nesting=1)
def logfire_source(
    read_token: Optional[str] = dlt.secrets.value,
    starting_at: Optional[str] = None,
) -> Any:
    """Load span/trace data from the Pydantic Logfire Query API.

    Args:
        read_token: Logfire project read token. Auto-loaded from secrets.toml.
        starting_at: Lower bound (ISO8601) for start_timestamp. Defaults to 7 days ago.

    Example:
        pipeline.run(logfire_source())
        pipeline.run(logfire_source(starting_at="2026-07-01T00:00:00Z"))
    """
    if starting_at is None:
        starting_at = pendulum.now("UTC").subtract(days=7).to_iso8601_string()

    config: RESTAPIConfig = {
        "client": {
            "base_url": "https://logfire-eu.pydantic.dev/v2/",
            "auth": {
                "type": "bearer",
                "token": read_token,
            },
